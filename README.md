...
An end-to-end NLP pipeline — from scraping real Indonesian-language app reviews to a deployed customer-service chatbot — built for the PPKD Jakarta Selatan Data Analyst program.

![Chatbot UI](screenshots/chatbot-ui.png)

📄 Full project report: [`laporan-proyek.pdf`](laporan-proyek.pdf)
...
| Notebook | Task |
|---|---|
| `T1_Scraping.ipynb` | Scrape 11,000 reviews via `google-play-scraper` |
| `T2_Labeling.ipynb` | Lexicon-based sentiment labeling with negation handling |
| `T3_Preprocessing.ipynb` | Case folding, regex cleaning, custom slang normalization (e.g. "wd"→"tarik", "tf"→"transfer"), Sastrawi stop-word removal & stemming |
| `T4_FeatureExtraction.ipynb` | Bag-of-Words vs. TF-IDF comparison (max_features=500) |
| `T5_Classification.ipynb` | Naive Bayes vs. SVM (LinearSVC) benchmarking |
| `T6_DistilBERT.ipynb` | Pretrained transformer baseline comparison |
| `T7_WordCloud.ipynb` | Visual sentiment-driver analysis |
...
## Chatbot Prototype (`src/`)
A working **"DANA Automated Resolution Center"** prototype using a **Hybrid NLP Routing** architecture:
- **Frontend:** `index.html` / `style.css` / `script.js`
- **Rule layer:** `chat.php` — PHP regex/keyword intent matching for known patterns (top-up, pending status, transfer errors, fraud alerts, account freezes) and Ticket-ID generation
- **ML fallback layer:** `app.py` — Python Flask API serving the saved SVM model (`svm_sentiment.pkl` + `tfidf_vectorizer.pkl`) for messages the rule layer can't confidently classify
